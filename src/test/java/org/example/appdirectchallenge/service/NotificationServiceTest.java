...
public class NotificationServiceTest {
    ...
    public void notificationService_change_somethingWasChanged_returnTrue() {
        when(oAuthClient.getNotification("url")).thenReturn(buildNotification());
        when(subscriptionRepository.update(new Subscription(1L, "CompanyName", "FREE", "ACTIVE", "http://example.org"))).thenReturn(true);
    ...
    public void notificationService_change_nothingWasChanged_returnFalseAndAccountNotFound() {
        when(oAuthClient.getNotification("url")).thenReturn(buildNotification());
        when(subscriptionRepository.update(new Subscription(1L, "CompanyName", "FREE", "ACTIVE", "http://example.org"))).thenReturn(false);
